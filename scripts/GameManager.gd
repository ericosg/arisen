...
# --- GAME STATE ENUMS ---
enum GamePhase { NONE, OUT_OF_TURN, TURN_STARTING, PLAYER_PRE_BATTLE, BATTLE_IN_PROGRESS, PLAYER_POST_BATTLE, WAVE_ENDING, TURN_ENDING }
...
const CREATURE_SCRIPT_PATHS = {
	...
	"Alien_Wasp": "res://scripts/creatures/Alien.gd", "Alien_Spider": "res://scripts/creatures/Alien.gd", 
	"Alien_Scorpion": "res://scripts/creatures/Alien.gd", "Alien_Beetle": "res://scripts/creatures/Alien.gd",
}
...
func start_new_game(): # ... (same as gamemanager_gd_v5)
	...
	player_undead_roster.clear(); available_corpses.clear()
	if is_instance_valid(battle_grid_node): battle_grid_node.initialize_grid_data() 
	if not is_instance_valid(necromancer_node): printerr("GM: Necro missing!"); return
	if not is_instance_valid(battle_grid_node): printerr("GM: BG missing!"); return
	if not is_instance_valid(units_container_node): printerr("GM: Units cont. missing!"); return
	necromancer_node.assign_runtime_references(self, battle_grid_node) 
	_change_game_phase(GamePhase.OUT_OF_TURN)

func proceed_to_next_turn(): # ... (same as gamemanager_gd_v5)
	if current_game_phase != GamePhase.OUT_OF_TURN and current_game_phase != GamePhase.TURN_ENDING and current_game_phase != GamePhase.NONE: return
...
func proceed_to_next_wave(): # ... (same as gamemanager_gd_v5)
...
func player_ends_pre_battle_phase(): # ... (same as gamemanager_gd_v5)
	if current_game_phase != GamePhase.PLAYER_PRE_BATTLE: return
	_initiate_battle_phase()

func _initiate_battle_phase(): # ... (same as gamemanager_gd_v5)
...
func player_ends_post_battle_phase(): # ... (same as gamemanager_gd_v5)
	if current_game_phase != GamePhase.PLAYER_POST_BATTLE: return
	_end_current_wave()

func _end_current_wave(): # ... (same as gamemanager_gd_v5)
...
func _end_current_turn(): # ... (same as gamemanager_gd_v5)
...
func _set_human_civilian_population(value: int): # ... (same as gamemanager_gd_v5)
...
func _change_game_phase(new_phase: GamePhase): # ... (same as gamemanager_gd_v5)
	if current_game_phase != new_phase: current_game_phase = new_phase

func _set_game_over(reason: String, msg: String): # ... (same as gamemanager_gd_v5)
...
func _auto_place_units(units_to_spawn_data: Array[Dictionary], faction: Creature.Faction): # ... (same as gamemanager_gd_v5)
...
func _prepare_creature_node_base() -> Node2D: # ... (same as gamemanager_gd_v5)
	var creature_base_node = Node2D.new()
	var sprite_node = Sprite2D.new(); sprite_node.name = "Sprite" 
	creature_base_node.add_child(sprite_node); return creature_base_node

func spawn_reanimated_creature(config_from_spell: Dictionary) -> Creature: # ... (same as gamemanager_gd_v5, but with explicit placeholder paths)
	...
	actual_creature.game_manager = self; actual_creature.battle_grid = battle_grid_node
	...
	if not final_config.has("sprite_texture_path"): # Ensure path is set
...
func _create_creature_node_from_config(type_key: String, config: Dictionary, fact_override: Creature.Faction) -> Creature: # ... (same as gamemanager_gd_v5, but with explicit placeholder path default)
	...
	actual_creature.game_manager = self; actual_creature.battle_grid = battle_grid_node
	var final_cfg = config.duplicate(true); final_cfg["faction"] = fact_override 
	if not final_cfg.has("sprite_texture_path") or final_cfg["sprite_texture_path"] == "res://icon.svg": # If still default or missing
		final_cfg["sprite_texture_path"] = "res://assets/images/YOUR_DEFAULT_SPRITE_FOR_%s.png" % type_key.to_upper() # Make it obvious
...
# ... (rest of GameManager.gd is the same as gamemanager_gd_v5)
...
func _resolve_combat_in_lane(col_idx: int) -> bool: # ... (same as gamemanager_gd_v5)
...
func player_deploys_undead_from_roster(undead: Creature, grid_pos: Vector2i) -> bool: # ... (same as gamemanager_gd_v5)
...
func player_returns_undead_to_roster(undead: Creature) -> bool: # ... (same as gamemanager_gd_v5)
	if current_game_phase != GamePhase.PLAYER_PRE_BATTLE: return false
